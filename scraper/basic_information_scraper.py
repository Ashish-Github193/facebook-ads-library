...
def get_advertiser_information_with_information_block(driver: webdriver.Chrome,
                                                       ...
                                                       advertisement_xpath: str):

    element_xpath = find_advertiser_name_element_xpath (parent_element=parent_element,
                                                        parent_element_xpath=advertisement_xpath)

    with suppress(Exception):
        element = driver.find_element(By.XPATH, element_xpath)
        return hover_element(driver=driver, element=element, xpath=advertisement_xpath)  # type: ignore

    logging.warning(f"Message: No ELEMENT found to hover on | Scraper: Basic information scraper | Function: get_advertiser_information_with_information_block | More info: xpath: {element_xpath}")
    return None

def scrape_basic_information (driver: webdriver.Chrome,
                              parent_element: WebElement,
                              advertisement_xpath: str):
    
    html = get_advertiser_information_with_information_block(driver=driver,
    ...
    logging.warning("Message: No HTML found | Scraper: Basic information scraper | Function: scrape basic information")
