...
def group_scraped_data ( driver: webdriver.Chrome, index: int, number_of_elements: int ) -> BeautifulSoup:  # type: ignore
    ...
    for advertisement_xpath in get_xpath_for_each_element(xpath_head=xpath_head,
        ...
        try: parent_element = driver.find_element(By.XPATH, advertisement_xpath)
        except: yield [None, None]

        advertiser_basic_information = scrape_basic_information(driver=driver,
        ...
        if advertiser_basic_information is not None:
            advertising_platform_names = get_advertising_platforms(parent_element=parent_element)
...
def scrape_data(driver, index: int, number_of_elements: int):
    for scraped_data in group_scraped_data (driver=driver,
                                            index=index,
                                            number_of_elements=number_of_elements):
