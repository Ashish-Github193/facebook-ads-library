...
from utils2 import check_if_start_is_bigger, get_current_date, get_min_date
...
while check_if_start_is_bigger(start_date=start_date_max_itr, end_date=END_DATE):
    ...
    # check if no companies
    if company_block_iteration_max_index < company_block_iteration_index: continue

    print()
    print()
    print(f"Scraping data from date: {start_date_min_itr} to date: {start_date_max_itr}")
